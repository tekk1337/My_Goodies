...
Get-CimInstance Win32_Processor | ft DeviceID,NumberOfCores,NumberOfLogicalProcessors
...
Get-CimInstance -Class Win32_logicaldisk -Filter "DriveType = '3'" | Select-Object -Property DeviceID, VolumeName, @{L='FreeSpaceGB';E={"{0:N2}" -f ($_.FreeSpace /1GB)}}, @{L="Capacity";E={"{0:N2}" -f ($_.Size/1GB)}} | Out-Default
...
If ($tls -eq $null){Write-Host "Unable to find Enabled Ciphers or Protocols"}Else{$tls}
